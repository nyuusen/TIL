...
- データ形式を示す
- レスポンスではContent-Typeヘッダで指定する
- メディアタイプは、MIME(Multipurpose Internet Mail Extensions)タイプとも呼ばれ、電子メールの仕様が由来している
- パソコンでは拡張子を使ってデータ形式を判断することが多いが、メールやウェブではこのメディアタイプを使ってデータ形式を表す
- メディアタイプは、`トップレベルタイプ名 / サブタイプ名 [; パラメータ]`で記述される
  - トップレベルタイプ名では、textとapplicationの違いが若干わかりづらいが、その形式を知っていないと読みこなせないものは、applicationとするのが主流となってきている
- Content-Typeを指定する必要性
  - クライアントの多くは、Content-Typeを読み取ってデータ形式をまずは判断するので、とても重要

### リクエストデータのメディアタイプ

- リクエストの際にもContent-TypeとAcceptがメディアタイプ関連のヘッダが使用される
- Content-Typeは、リクエストボディがどのようなデータ形式で送られているか
  - フォームデータで使用されるのがapplication/x-www-form-urlencodedである
...
- SOPによる同一生成元ポリシー
- それを可能とするのがCORS
- CORSを行うには、まずクライアントからOriginというリクエストヘッダを送る必要がある
  - クライアント側のベースURL部分が入る
- サーバー側ではあらかじめアクセスを許可する生成元（URL）一覧を持っておき、そのリストに含まれるかをチェックする
  - チェックがOKであれば、Access-Control-Allow-Originに、Originと同じ値をセットすることでアクセスが許可されたことを示す
  - もしくはどこからでも読まれて良い場合は、`*`をセットする
- CORSとユーザー認証情報
...
## 5章 設計変更をしやすいWebAPIを作る

- ひとまずAPIの設計（IF）を変更するのはとても大変なことであるというが大前提
  - 特に外部に公開しているようなAPIの場合
- 一度公開したAPIをできる限り変更しない方法
  - APIをバージョンで管理する
    - URIのパスに`v1`といったようなバージョン番号を付与する
      - バージョン管理手法はセマンティクスバージョニング
        - コラムとして、バージョン番号を日付で表すのは伝統的な方法（AWSがいまだに日付をバージョンとしている）
        - 現在は一般的ではない、あえて使う必要もない
    - クエリ文字列に含めるという選択肢もある
      - ただクエリ文字列は省略可能であり、省略時にどのバージョンを示すのかが自明でないこと、見た目が冗長になることから、URLパス案の方が良い
- オーケストレーション層
  - 様々なデバイスの機能への対応を入れたものごとにAPIを提供するのは大変
  - そこでNetflix社では、オーケストレーション層としてClient Adapter Codeを実行する層を挟んでいる
  - この層で色々な差分を吸収することで、ユーザー体験を最適化することができる

## 6章 堅牢なWebAPIを作る

- 安全性と安定性の面から考えてみる章
- HTTPS化
  - 暗号化される対象はURIパス、クエリ文字列、ヘッダとボディなどやり取りに使われるほぼ全ての情報
- HTTPSを使えば100％安全か？
  - そうではない
  - 中間者攻撃: クライアントとサーバー間に入り込んで、中継したサーバで情報を盗み出す
    - これにはサーバが送ってきた証明書をきちんと検証することで、そのような怪しい証明書を検知することができる
  - 大抵のHTTPSをサポートするHTTPクライアントライブラリでは、こうした検証を行なってくれることは多い
    - ただしコモンネーム（証明書のフィールドで証明書が有効な対象のFQDN）の検証を標準では行わないライブラリも多いので注意
    - (追記)CNには1つしかドメインを設定できないという制約があるので、複数のドメインを設定できるSAN(SubjectAlternativeName)を使用するのが主流
      - 共存することはあるが、SANがあるならSANのみを検証対象とするブラウザやクライアントが多い
      - SANには、ワイルドカードを設定することができ、1つの証明書であらゆるサブドメインに対応することが可能
...
- XSS
  ...
  - Content-Typeを正しく設定する
    ...
    - IE7以前の対応方法
      - JSON文字列エスケープ
        - JSON文字列で許可されていないのは`"`と`\`とコントロール文字でそれ以外のすべてのユニコード文字は格納可能
        - だが、たとえば`/`をエスケープすることで、`<\/script>`のように要素の閉じタグを無効にすることができる
          - これはインラインスクリプトで、エスケープ未処理の`</script>`が含まれていると、HTMLパーサがそれをスクリプトの終了タグだと誤認し、XSSが発生してしまうリスクがある（それを避けるために / を \/ とエスケープし、パーサの誤認を防ぐ）
          - 逆にAjaxでのJSONフェッチならこの考慮は不要ではある（JS内で処理する分にはHTMLパーサが介在しないため）
          - さらに`<`や`>`もエスケープすることでscript要素を完全に無効にできる
      - 追加のリクエストヘッダ検証
        - 通常JSONでのデータのやり取りはXMLHttpRequestで行われるので、何かしらのカスタムヘッダをつけることが可能
        - jQueryでは`X-Requested-With: XMLHttpRequest`を送る仕様となっている
- XSRF
  - フォージェリ: 偽造、捏造
  - ユーザーが悪意あるサイトに訪問した時に埋め込まれたリンクやフォーム、JavaScriptなどを経由して、全く別のサイトにリクエストが送られてしまうこと
      - 掲示板に勝手に投稿、銀行口座に勝手に送金など..
  - 対策
    - サーバー側のデータが変わるようなアクセスに関してはGETメソッドを利用しない
      - img要素などを用いて攻撃用のコードを埋め込むことができなくなる
      - そもそもマナー的にも、クローラーがアクセスした時に悪さしないためにも必ずやっておく
      - ただしGETを使わなかったとしてもform要素はSOP制限を受けないため、POSTリクエストは可能
          - XSRFトークン
              - サーバー側でトークン発行が必要となり、やや煩雑にはなる
          - カスタムヘッダをつける
              - XMLHttpRequestであれば
              - form要素の場合はヘッダは独自につけられない
      - SPA構成でトークンベースの認証方式の場合はCookieを使わないのでCSRFリスクはあまりない
        - ブラウザが勝手にAuthorizationヘッダをつけるわけではないので
- JSONハイジャック
  - script要素にはSOPが適用されない
  - ただし近年のブラウザは Content-Type: application/json に対して <script>タグでは実行されないようになっている。
  - CORSの導入が進んだ。
  - JSONPの廃止。
  - フレームバスター、CSRF対策なども一般化。
- 認証されたユーザーによる不正行為
  - パラメータ改ざん
    - アイテム消費APIでマイナスの値を指定することで無限に増やしたりなど
    - クライアントから送られてきた情報は信頼せずにサーバー側で整合性をきちんとチェックする
      - アイテムの個数は適切な範囲にあるか？
- リクエストの再送信
  - APIごとに繰り返し送信されたら問題があるかを判断する
  - 問題がある場合は、状態を管理（ゲームにおける開始・経過・終了を記録したり）するようにし、同じアクセスが何度もあった場合はエラーにするなどの対応にする
- 支払い偽装
