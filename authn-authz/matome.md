...
前提として、認証方式（そもそも認証どう実現する？）と認証済みである情報をどうクライアントに持たせるか？の2軸の話がある。  
...
認証済みである情報をどうクライアントに持たせるか？については、認証方式ごとにある程度型が決まっている。大きく分けるとセッション(Cookie)ベースとトークンベースが存在する。
- ID・パスワード認証：セッション（認証成功時にサーバー側でセッション管理）
- Basic認証：毎回送信＆検証（同一セッションの場合はブラウザが記憶する）
- ダイジェスト認証：毎回送信＆検証（同一セッションの場合はブラウザが記憶する）
- SAML：セッションが多いが、JWTなどのトークンベースであることもある（これはSAMLアサーションを受け取ったSPの実装次第）
- OIDC：トークンベース（JSからアクセスしやすいWebStorageか、XSS対策でHttpOnlyなCookieであることもままある）

従来のMPA構成の場合は、認証状態をサーバー側でセッションとして中央集権的に管理することが多い。  
なぜなら、MPA構成の場合はリクエストごとにシンプルに単一のHTMLを返すので、自動送信されるCookieと相性が良い（逆にAuthorizationヘッダに値をセットする必要があるものとは相性が悪い）

昨今のSPA（またはSPAライク）の場合は、どちらでも良いがトークンベースが採用されることが多い（気がする）  
...
- JWTには、Header（トークン種類や署名アルゴリズム）とPayload（ユーザーID、有効期限、ロール等）とSignature（ヘッダとペイロードを暗号鍵で署名し、改竄されていないことを保証する）
...
- 署名検証処理について
  - まず署名自体は、サーバー側でヘッダーとペイロードを秘密鍵を用いて暗号化する
    - 署名アルゴリズムは、秘密鍵を使ってHMAC-SHA256（対称鍵署名）でハッシュ化したもの
  - 署名検証は、JWTのSignatureとHeader+Payload+秘密鍵から計算される署名を比較し、一致するかどうかで判断
  - もし一致しない場合は、Payloadが書き換えられていると判断される
  - クライアント側ではPayloadの書き換え自体は可能ではあるが（Base64エンコードされているだけなので）、署名の書き換えは不可能（秘密鍵を知らないので）ので、この仕組みで改ざんを防止している
- ちなみに外部IDaaS（GoogleやOkta等）が返すid_tokenやaccess_tokenの値は、このJWT形式であることが多い
  - id_tokenとは？
    - **認証**情報
    - JWT形式
    - 使い道としては、クライアントアプリ側でデコードして、ユーザー名を取得し、
    - APIには送られない
  - access_tokenとは？
  - refresh_tokenとは？
