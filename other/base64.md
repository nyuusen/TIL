...
## じゃあBase64エンコードしなくて済むmultipart/form-data使えば良いのでは？

- JSON(+Base64)で統一したい
  - フロントエンド側はJSON.stringifyやるだけで済むし、サーバー側のパーサもJSON想定してパースするだけで済む
  - Multipartの場合は、FE側はFormData APIを使って、JSONなどの構造化データとは分ける必要があったりと実装が面倒
  - こんなことするくらいなら「33%の容量増なんてええかー」という話
- ミドルウェアとの相性
  - 詳しく調査はしていないが、WAFやAPI Gatewayでの処理で時間がかかったりスキップされたり..
- データが非常に小さい場合はオーバーヘッド分だけサイズが大きくなる可能性？
  - パートごとのBoundary（境界文字列）を付与するだけで数百バイトのオーバーヘッドが発生する
- FEエコシステムの制約
  - Base64だとただの文字列プロパティとしてWebStorageやHTMLやCSSのsrc属性にそのまま埋め込める
...
- CPUやメモリへの負荷
  - エンコード・デコード
  - プログラム内で文字列としてBase64を保持すると、元のバイナリに加えてその1.3倍の長さの文字列をメモリ上に置く必要がある
- WAFとの相性
  - 偶然Baseエンコード値の中にSQLインジェクションのようなパターンが出現すると誤ってブロックしてしまう恐れも
- URLエンコードの必要性
  - Base64文字列をURLクエリパラメータに乗せる場合は/や+をURLエンコードする必要がある