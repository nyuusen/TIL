...
- セカンダリインデックスは2ホップでのデータ取得になる
  - セカンダリインデックスで主キー特定→主キーでデータを引く
  - 代替として主キーの変更やカバリングインデックスなど

### DDL

https://docs.pingcap.com/ja/best-practices/ddl-introduction/

- TiDBは、DDL文の実行にオンラインかつ非同期のアプローチを採用している
  - DDL実行中にデータ変更を送信することができ、データベースはデータの一貫性と正確性を保証する
  - オフラインDDLはサポートしていない
- DDLは2種類存在する
  - 論理DDLステートメント: テーブル名や列名の変更など、オブジェクトに格納されているデータを処理せずに、データベースオブジェクトのメタデータのみを変更する
    - 通常実行時間が短く、完了までに数十ミリ秒・数秒しかかからないことも多い
    - システムリソースをあまり消費せず、ワークロードにも影響を与えない
  - 物理DDLステートメント: メタデータを変更するだけではなく、オブジェクトに格納されているユーザーデータも変更する
    - 例えば、新たにインデックスを作成する場合、インデックスを構築するためにテーブルを全スキャンする、カラム型の変更
    - DDL実行には、TiKVのCPUとI/O（データ読み取りと新規データの書き込み）とTiDBのCPU（DDLオーナーとタスクを実行するそれぞれのノード）のリソースを消費するため、ワークロードに影響を与える

#### ベストプラクティス

-  DDL操作を行うワーカー数とデータ量をシステム変数で設定し、アプリケーション負荷への影響をコントロールする
   - 他に負荷があればこれらの数を減らしてアプリケーション側の影響を最小限に抑える、逆に他に負荷がなければ数を増やすことでADD INDEXなどの処理速度を上げる
 - DDLリクエストを同時に送信して、多数のテーブルを素早く作成する
   - テーブル作成操作には約50m秒かかる
   - 同時にリクエストを送ることで、テーブル作成速度を最大化できる
 - 1つのALTER文で複数の変更を行う
   - 単一のALTER文でテーブル内の複数のスキーマオブジェクトを変更できる
   - 文全体はアトミック性維持される
   - なので複数の変更は、単一のALTER文で行うことが推奨

### インデックス作成のベストプラクティス

- カバリングインデックスで、インデックス上で直接データをクエリする
- 頻繁にクエリを実行しない列には、セカンダリインデックスを作成しない
  - インデックスを作成すると、行を挿入する際にキーと値が1つずつ追加される
  - なので、書き込み速度が低下し、消費する領域も増加する
  - さらにインデックスが多すぎると、オプティマイザの実行時間への影響やオブティマイざに誤動作をさせる可能性が

### その他

> Replace OR with IN or UNION. The number of IN must be less than 300.

クエリ例: `WHERE a = 1 OR b = 2` (a,bともにインデックスが貼ってある前提)

- ORは、結果のマージコストが発生し、これはCPUコストが高く、重複排除が必要になるため、フルスキャンの方が早いと判断されやすい
- ORは、どちらが一方が真であれば成立するため、評価順序によってコストが変わるが、オプティマイザが最適な評価順序を推測するには、統計情報が完璧でない限り困難
  - →つまり、ORを多用したクエリはインデックスが効かなくなり、フルスキャンにフォールバックする可能性が高まる
- UNIONやINに置き換えるべき理由
  - オプティマイザは、それぞれのSELECTクエリに対して独立して最適な単一インデックスを選択できる
  - TiDBにおいては、それぞれが独立したクエリを並列で動かせるので、分散システムの並列性を最大限に活かせる
- 同一カラムに対するOR検索であれば、INにすることでオプティマイザはそれを単一の多点範囲検索として扱う

### UUIDを主キーとして扱う時にベストプラクティス
...
- VARCHAR(36)だと36バイト、BINARY(16)なら16バイトで半分以下なので、1ページ(ブロック)に入るデータ量に差があるので、ディスクから読み出すページ数が減り、I/Oボトルネックが緩和される（メモリに多くキャッシュできるようになる）
- 文字列の比較は、文字コード（コレーション）に従って先頭から1文字ずつASCIIコードを比較するので、CPU命令数が多くなる
  - バイナリは内部的には生ビット列なので、低コスト・高速に処理できる