...
### JavaScriptにおける宣言と代入の違い

- 宣言：function構文
- 代入：ラムダ関数を作成し変数に代入
- 違い①評価のタイミング
  - 巻き上げ・Hoistingとも言う
  - function構文の場合は、コンパイラやインタープリタが「ここに変数がある」と認識し、先にスキャンしてメモリを確保するため、定義する前の行でも呼び出して動く
  - 一方、変数代入の方は、変数は単純に上から下に評価されるため、代入する前の行で呼び出すとエラーとなる
- 違い②名前（識別子）の所有権
  - function構文の場合は、関数自体が自分の名前を知っているので、再帰的に自分を呼び出すことなどが可能
  - 一方、変数代入の方は、関数自体は名前を知らず、たまたま変数に入っているだけ

## クロージャ

- クロージャとは、関数とその関数が定義された環境（スコープ）をセットで閉じ込めたもの
  - 通常、関数の内部で定義された変数は、関数の実行が終わるとメモリから消えるが、クロージャを使うとその内部にいた変数を生かし続けることができる
- 
...
- 呼び出し可能なオブジェクトはデータのように扱うことができ、関数を入力として受け取り、関数を出力として返す関数（一方もしくは両方を満たす関数）を「高階関数」という
  - 身近な例としてはmap,filter,forEachやgoのhttp.HandlerFunc
    - データの集まりをループ処理するという共通の処理と、そのデータに何をするかという個別のロジックを分離できるのが嬉しい点

## コールバック

- ある関数Fが関数Cを入力として受け取り、関数F内のどこかで呼び出す場合、関数Cは「コールバック関数」と呼ばれる（関数Fは高階関数）
  - 関数Cはすぐに実行されることもあれば、非同期関数のように後で呼び出されることもある
