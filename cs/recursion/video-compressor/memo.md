...
- パイプが一方向のデータストリームであることに対し、ソケットは双方向のデータストリーム
- 異なるコンピュータ間でプロセスが通信するために設計された
- ソケット自体は通信の「口」「窓口」であり、アプリケーションがネットワークを介して送受信するためのもの
- Goなどのプログラムから見ると、ソケットはファイルディスクリプタとして扱われるが、その実体はTCP・UDPなどのネットワークプロトコルに関連づけられた特殊な通信リソース
- サーバークライアント間通信
  - サーバー側アプリケーションが、特定のIPアドレスとポート番号に素血tを作成し、待ち受け状態になる（listen操作）
  - クライアント側アプリケーションが、サーバーのIPアドレスとポート番号を指定してソケットを作成し、接続を試みる（connect操作）
  - 接続が確立し、両端のソケットは双方向のデータストリーム（I/Oストリーム）となり、同時にデータの読み書きが可能になる（read/write操作）
- トランスポート層:TCPかUDPか
  - アプリケーションが通信する方法はソケットタイプで決定する

## RPC

- RPCは、あるプログラムが別の場所や別のプログラムに対して仕事を依頼するための方法であり、具体的には自分のプログラムから他の場所やマシン上のプログラムの関数を呼び出す手段となる
- RPCライブラリやフレームワークは、ソケット接続・データのシリアライズ/デシリアライズ・コード生成など、基盤となる低レベルのコードのほとんどを実装している
- Web API(REST)と比較して嬉しい点
  - データ形式が軽量
    - JSONと比較すると、Protocol Buffersなどのバイナリ形式はメッセージサイズが小さく、パース処理も高速
  - 効率的なプロトコル
    - RPCは、HTTP/2をトランスポート層として使用し、HTTP/2はヘッダー圧縮や多重化の機能を持つため、HTTP/1.1を使用するRESTよりもネットワーク効率が良い
  - シンプルで厳密なサービス定義
    - 事前に定義されたサービスインターフェース(IDL:Interface Definition Language)で開発がシンプルでエラーが減る
    - 関数を呼び出す感覚で
    - IDLファイルでデータ構造とサービスメソッドを定義するとコードを自動生成できるので、手作業によるエラーの排除とクライアントサーバー間の整合性が保証される

### 関数の実行とパラメータの管理

- サーバがリクエストを受け付けるとどの関数を実行するか特定する必要がある
  - サーバーはstring,callable型のハッシュマップを目次として作成し、リクエストに指定されたキーに関連づけられた関数を探す
